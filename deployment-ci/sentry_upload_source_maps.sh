...
npx sentry-cli --auth-token "$SENTRY_AUTH_TOKEN" releases -o "$SERVER_SENTRY_ORG" -p "$SERVER_SENTRY_PROJECT" files "$SENTRY_RELEASE_VERSION" \
...
npx sentry-cli --auth-token "$SENTRY_AUTH_TOKEN" releases -o "$CLIENT_SENTRY_ORG" -p "$CLIENT_SENTRY_PROJECT" files "$SENTRY_RELEASE_VERSION" \
